...
class LogServerHandler(SimpleHTTPRequestHandler):
    ...
    def send_experience_content(self, date_suffix, player_name):
        ...
        if not os.path.exists(exp_dir):
            self.send_json_response(
                {'error': 'Experiences directory not found', 'content': ''})
            return
        ...
        if not found_file:
            self.send_json_response(
                {'error': 'Experience file not found', 'content': '', 'player': player_name})
            return
        ...
        try:
            ...
            self.send_json_response(
                {'error': str(e), 'content': '', 'player': player_name})
    ...
    def _append_termination_log(self, status="异常终止"):
        """在最新日志末尾追加终止信息，避免重复追加。"""
        log_path = self._get_latest_log_path()
        if not log_path:
            return False
    ...
    def stop_game(self):
        ...
        try:
            ...
            logged = self._append_termination_log(status="异常终止")
            game_process = None
            msg = '游戏已停止'
            if logged:
                msg += '，日志已记录'
